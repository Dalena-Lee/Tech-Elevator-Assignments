...
public class Exercise03_Shirts {
    ...
    public char[] buildBulkOrder(int numberOfShirts) { 
        //Create an array for the distribution
        char[] distributedShirts = new char[numberOfShirts];

        //Create a for loop to iterate through the numberOfShirts
        for (int i = 0; i < numberOfShirts; i++){
            //if index[i] has a remainder of 2, make the previous two indices the correct size.
            if (i % 3 == 2) {
                distributedShirts[i] = LARGE_TSHIRT;
            } else if (i % 3 == 1) {
                distributedShirts[i] = MEDIUM_TSHIRT;
            } else {
                distributedShirts[i] = SMALL_TSHIRT;
            }
        }

        return distributedShirts;
    }
    ...
    public boolean placeRequest(char[] order) {
        //Create a for loop that scans each index.
