...
public class Exercise05_Weather {
    ...
    */
    public int belowFreezing(int[] dailyHighs) {
        //Declare a variable to count the days at or below freezing.
        int numOfDaysFreezing = 0;
        //Create a for loop to iterate through dailyHighs.
        for (int i = 0; i < dailyHighs.length; i++) {
            if (dailyHighs[i] <= 32) {
                numOfDaysFreezing += 1;
            }
        }
        return numOfDaysFreezing;
    }
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        int mostHotDay = dailyHighs[0];
        //Create a for loop to iterate through dailyHighs
        for (int i = 0; i < dailyHighs.length; i++) {
            if (mostHotDay < dailyHighs[i]) {
                mostHotDay = dailyHighs[i];
            }
        }
        return mostHotDay;
    }
