...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        if (unitsUsed > BASIC_SERVICE_LIMIT) {
            return ((BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE));
        }
        else {
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        if (unitsUsed > BASIC_SERVICE_LIMIT) {
            if (hasRenewableEnergy) {
                return ((BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE)) - (((BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE)) * RENEWABLE_ENERGY_DISCOUNT);
            } else {
                return ((BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE));
            }
        }
            ...
            if (hasRenewableEnergy) {
                return (unitsUsed * BASIC_SERVICE_RATE) - ((unitsUsed * BASIC_SERVICE_RATE) * RENEWABLE_ENERGY_DISCOUNT);
            } else {
                return unitsUsed * BASIC_SERVICE_RATE;
            }
        }
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        double overServiceLimit = BASIC_SERVICE_LIMIT * BASIC_SERVICE_RATE;
        double discountOverLimit = ((overServiceLimit) + (((netUsage) - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE)) * RENEWABLE_ENERGY_DISCOUNT;
        double discountUnderLimit = (BASIC_SERVICE_RATE * (netUsage)) * RENEWABLE_ENERGY_DISCOUNT;
        double ifLessThanLimit = BASIC_SERVICE_RATE * netUsage;

        if (unitsReturned > 0) {

            if (netUsage > BASIC_SERVICE_LIMIT) {
                billAmount = ((overServiceLimit) + (((netUsage) - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE)) - (discountOverLimit);
                if (billAmount > 0) {
                    return ((overServiceLimit) + (((netUsage) - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE)) - (discountOverLimit);
                }
            } else {
                billAmount = (ifLessThanLimit) - (discountUnderLimit);
                if (billAmount > 0) {
                    return (ifLessThanLimit) - (discountUnderLimit);
                } else {
                    return ifLessThanLimit;
                }
            }
        }
        else {
            if (unitsUsed > BASIC_SERVICE_LIMIT) {
                billAmount = ((overServiceLimit) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE));
                if (billAmount > 0) {
                    return ((overServiceLimit) + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE));
                }
            } else {
                billAmount = unitsUsed * BASIC_SERVICE_RATE;
                if (billAmount > 0) {
                    return unitsUsed * BASIC_SERVICE_RATE;
                }
            }
