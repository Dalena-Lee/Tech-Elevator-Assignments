...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            if (hasDiscount) {
                return (weightPounds * UP_TO_40_LB_RATE) - ((weightPounds * UP_TO_40_LB_RATE) * 0.10);
            } else {
                return (weightPounds * UP_TO_40_LB_RATE);
            }
        } else if (weightPounds > MAX_WEIGHT_POUNDS) {
            if (hasDiscount) {
                return (MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE) - (((MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE)) * 0.10);
            } else {
                return (MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE);
            }
        }
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            return (weightPounds * UP_TO_40_LB_RATE) - ((weightPounds * UP_TO_40_LB_RATE) * discountPercentage);
        } else if (weightPounds > MAX_WEIGHT_POUNDS) {
            return (MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE) - (((MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE)) * discountPercentage);
        }
        return 0;
    }
}
